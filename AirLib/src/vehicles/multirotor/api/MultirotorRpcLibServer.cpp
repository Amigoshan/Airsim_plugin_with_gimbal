        ...
        : RpcLibServerBase(drone, server_address, port)
{
	...
	(static_cast<rpc::server*>(getServer()))-> // gimbal api -- amigo
		bind("rotateCameraBySpeed", [&](int cam_id, float v_roll, float v_pitch, float v_yaw, float duration) ->
			bool { return getDroneApi()->rotateCameraBySpeed(cam_id, v_roll, v_pitch, v_yaw, duration); });
	(static_cast<rpc::server*>(getServer()))->
