...
public:
    struct State {
		...
		// camera pose for gimbal control -- amigo
		Vector3r cam_angle;
		Vector3r cam_velocity;
        ...
        static State zero()
        {
			...
			// initiallize cam orientation, only single camera supported -- amigo
			zero_state.cam_angle = Vector3r::Zero();
			zero_state.cam_velocity = Vector3r::Zero();
	...
	const Vector3r getCameraAngle() const // for simple gimbal velocity control -- amigo
	{
		return current_.cam_angle; 
	}

	const Vector3r getCameraVelocity() const // for simple gimbal velocity control -- amigo
	{
