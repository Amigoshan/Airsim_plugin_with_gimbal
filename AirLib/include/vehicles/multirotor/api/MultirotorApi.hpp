...
public:
	...
	// rotate the camera by speed -- amigo
	bool rotateCameraBySpeed(int cam_id, float v_roll, float v_pitch, float v_yaw, float duration)
	{
		std::shared_ptr<OffboardCommand> cmd = std::make_shared<RotateCameraBySpeed>(controller_, cam_id, v_roll, v_pitch, v_yaw, duration);
		return enqueueCommand(cmd);
	}
...
private:// types
	...
	// rotate camera by speed -- amigo
	class RotateCameraBySpeed : public OffboardCommand {
		int camid_;
		float vroll_, vpitch_, vyaw_;
		float duration_;
	public:
		RotateCameraBySpeed(DroneControllerBase* controller, int cam_id, float vroll, float vpitch, float vyaw, float duration) : OffboardCommand(controller) {
			this->camid_ = cam_id;
			this->vroll_ = vroll;
			this->vpitch_ = vpitch;
			this->vyaw_ = vyaw;
		...
		virtual void executeImpl(DroneControllerBase* controller, CancelableBase& cancelable) override {
			controller->rotateCameraBySpeed(camid_, vroll_, vpitch_, vyaw_, duration_, cancelable);
		}
	};
