...
public:
	...
	real_T getCamControlSignal(unsigned int axis_index) const //for gimbal control -- amigo
	{
		//convert PWM to scalled 0 to 1 control signal
		return static_cast<float>(cam_output_[axis_index]); // if the controller is off, it will return a default 0
	}
...
public:
	...
	virtual void writeCamOutput(uint16_t index, float value) override
	{
		cam_output_[index] = value;
	}
...
private:
	...
	std::vector<float> cam_output_; // for gimbal control -- amigo
