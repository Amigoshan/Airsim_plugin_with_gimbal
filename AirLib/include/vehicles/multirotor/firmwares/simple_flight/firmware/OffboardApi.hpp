...
public:
	...
	virtual const Axis4r getCamGoalValue() const override
	{
		return cam_goal_;
	}
	...
	// for gimbal control -- amigo
	virtual bool setCameraGoal(const float v_roll, const float v_pitch, const float v_yaw, std::string& message) override
	{
		if (has_api_control_) {
			OUTPUT("Camera goal Set !!!");
			cam_goal_[0] = v_roll;
			cam_goal_[1] = v_pitch;
			cam_goal_[2] = v_yaw;
			return true;
...
private:
	...
	Axis4r cam_goal_; // for gimbal control -- amigo
