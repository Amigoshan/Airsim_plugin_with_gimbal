...
public:
        ...
        offboard_api_(params, board, board, state_estimator, comm_link), mixer_(params)
    {
		...
		cam_controller_ = std::unique_ptr<GimbalController>(new GimbalController(params, board, comm_link));
		cam_controller_->initialize(&offboard_api_, state_estimator_);
    ...
    virtual void reset() override
    {
        ...
        controller_->reset();
        offboard_api_.reset();
		cam_controller_->reset(); // for gimbal control -- amigo
		
        motor_outputs_.assign(params_->motor.motor_count, 0);
		//cam_outputs_.assign(2, 0);
    }
    ...
    virtual void update() override
    {
		...
		if (cam_controller_ != nullptr)
		{
			cam_controller_->update();
			const Axis4r& output_cam_controls = cam_controller_->getOutput();
			for(unsigned int k=0; k<output_cam_controls.AxisCount(); k++) 
				board_->writeCamOutput(k,output_cam_controls[k]);
		}
...
private:
	...
	std::unique_ptr<IController> cam_controller_; // for camera

    std::vector<float> motor_outputs_;
	//std::vector<float> cam_outputs_; // for gimbal control -- amigo
};
