...
public:
	...
	virtual real_T getCamControlSignal(unsigned int axis_index) override // for gimbal control -- amigo
	{
		auto control_signal = board_->getCamControlSignal(axis_index);
		return control_signal;
	}
...
protected: 
	...
	// control the gimbal movement -- amigo
	virtual void commandCameraRotate(float v_roll, float v_pitch, float v_yaw) override
	{
		Utils::log(Utils::stringf("cameraRotate at speed %f %f, %f", v_roll, v_pitch, v_yaw));

		std::string message;
		firmware_->offboardApi().setCameraGoal(v_roll, v_pitch, v_yaw, message);
	}
