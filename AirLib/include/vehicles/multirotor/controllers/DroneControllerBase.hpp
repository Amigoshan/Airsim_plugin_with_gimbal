...
public: //interface for outside world
	...
	// Rotate the camera in roll, pitch and yaw angle -- amigo
	virtual bool rotateCameraBySpeed(int cam_id, float v_roll, float v_pitch, float v_yaw, float duration, CancelableBase& cancelable_action);
...
protected: //must implement interface by derived class
	...
	virtual void commandCameraRotate(float v_roll, float v_pitch, float v_yaw) // rotate camera in pitch and yaw -- amigo
