...
VehiclePawnWrapper::Pose VehiclePawnWrapper::getCameraPose(int camera_id)
{
	if (camera_id >= getCameraCount())
		return Pose::nanPose(); // the camera index exceed the total camera number

	APIPCamera* camera = getCamera(camera_id);
	return camera->getPose();
}

//set camera pose -- amigo
void VehiclePawnWrapper::setCameraPose(int camera_id, const Pose & pose)
{
	if (camera_id >= getCameraCount())
		return; // the camera index exceed the total camera number
